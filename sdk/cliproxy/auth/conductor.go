...
func (m *Manager) executeStreamWithModelPool(ctx context.Context, executor ProviderExecutor, auth *Auth, provider string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, routeModel, executionModel string, execModels []string, pooled bool, aliasResult OAuthModelAliasResult) (*cliproxyexecutor.StreamResult, error) {
	...
	for idx, execModel := range execModels {
		...
		execOpts := opts
		execReq, execOpts = applyRequestAfterAuthInterceptor(ctx, executor, auth, provider, execReq, execOpts, requestedModelAliasFromOptions(execOpts, routeModel))
...
func applyRequestAfterAuthInterceptor(ctx context.Context, executor ProviderExecutor, auth *Auth, provider string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, requestedModel string) (cliproxyexecutor.Request, cliproxyexecutor.Options) {
...
func (m *Manager) executeMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (cliproxyexecutor.Response, error) {
	...
	for {
		...
		for _, upstreamModel := range models {
			...
			execOpts := opts
			execReq, execOpts = applyRequestAfterAuthInterceptor(execCtx, executor, auth, provider, execReq, execOpts, requestedModelAliasFromOptions(execOpts, routeModel))
...
func (m *Manager) MarkResult(ctx context.Context, result Result) {
	...
	if auth, ok := m.auths[result.AuthID]; ok && auth != nil {
		...
		if result.Success {
			...
			if result.Model != "" {
				if !isRequestScopedResultError(result.Error) && !ignoreVisibleAliasModelFailure {
